(function()
{
	...
	movie_app.config(['$routeProvider', function($routeProvider)
	{
	    $routeProvider.when('/',
	    {
	        templateUrl: 'templates/movie_list.html', controller: 'MovieListController'
	    }).when('/movie_add',
	    {
	        templateUrl: 'templates/add_movie.html', controller: 'AddMovieController'
	    }).when('/:movieName',
	    {
	        templateUrl: 'templates/movie_details.html', controller: 'MovieDetailsController'
	    }).when('/:movieName/movie_edit',
	    {
	        templateUrl: 'templates/movie_edit.html', controller: 'EditMovieController'
	    }).otherwise(
	    {
	        redirectTo: '/'
	    });
 	...
 	movie_app.controller('AddMovieController', ['$scope', 'localStorageService', '$location', function($scope, localStorageService, $location)
 	{
	    ...
	    $scope.movie_add= function ()
	    {
		    $scope.movies.push(
		    {
	 			"name": $scope.name, "description": $scope.description, "image": $scope.image, "trailer": $scope.trailer, "imdb": $scope.imdb,
				"year": $scope.year, "director": $scope.director, "duration": $scope.duration, "genre": $scope.genre,
 	...
 	{	
		...
		$scope.movie_edit = function()
		{
			$scope.movies.push(
			{
	 			"name": $scope.movie.name, "description": $scope.movie.description, "image": $scope.movie.image, "trailer": $scope.movie.trailer,
				"imdb": $scope.movie.imdb, "year": $scope.movie.year, "director": $scope.movie.director, "duration": $scope.movie.duration,
				"genre": $scope.movie.genre, "actors": [$scope.movie.actor0, $scope.movie.actor1, $scope.movie.actor2]
 		 	});
	    	$location.path('/');
		};
	}]);
